...
async def dal_create_application(user_id: int, advertisement_id: int, proposed_advertisement_id: int | None, db: AsyncSession) -> Application:
    application = Application(user_id=user_id, advertisement_id=advertisement_id, proposed_advertisement_id=proposed_advertisement_id)
    db.add(application)
    await db.commit()
    await db.refresh(application)
    logger.info(f"Created application ID {application.id} for user_id {user_id} on advertisement_id {advertisement_id} with proposed_advertisement_id {proposed_advertisement_id}")
    return application
...
async def dal_get_applications_by_user_ads(user_id: int, db: AsyncSession) -> List[Application]:
    stmt = select(Application).join(
        Advertisement,
        Application.advertisement_id == Advertisement.id  # Явное указание условия соединения
    ).join(
        UserRole,
        ...
        selectinload(Application.proposed_advertisement)
            .selectinload(Advertisement.location)
            .selectinload(Location.street)
            .selectinload(Street.city),
        selectinload(Application.proposed_advertisement)
...
async def dal_delete_application_by_id(application_id: int, db: AsyncSession) -> None:
    ...
    application = result.scalar_one_or_none()
    if not application:
        logger.error(f"Application ID {application_id} not found for deletion")
...
async def dal_get_applications_by_user(user_id: int, db: AsyncSession) -> List[Application]:
    ...
    result = await db.execute(stmt)
    applications = result.scalars().all()
    logger.info(f"Retrieved {len(applications)} applications for user_id {user_id}")
    return applications


async def dal_get_applications_by_advertisement(advertisement_id: int, db: AsyncSession) -> List[Application]:
    stmt = select(Application).where(Application.advertisement_id == advertisement_id).options(
        selectinload(Application.user)
            .selectinload(User.user_logos)
            .selectinload(UserLogo.logo),
        selectinload(Application.advertisement)
    ...
    result = await db.execute(stmt)
    applications = result.scalars().all()
    logger.info(f"Retrieved {len(applications)} applications for advertisement_id {advertisement_id}")
    return applications